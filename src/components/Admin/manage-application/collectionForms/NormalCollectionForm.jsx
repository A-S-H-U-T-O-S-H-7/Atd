...
const NormalCollectionForm = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert('Failed to process collection. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const sanctionAmount = application?.sanctionAmount || "9000";
  const processFee = application?.processFee || "1274";
  const processFeeDetail = "(Principal: 1080 + GST: 194)";
  const disburseDate = application?.disburseDate || "2025-11-10";
  const transactionDate = application?.transactionDate || "2025-11-10";
  const dueDate = application?.dueDate || "2025-12-09";
