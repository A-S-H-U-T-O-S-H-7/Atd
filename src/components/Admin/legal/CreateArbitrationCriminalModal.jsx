...
const validationSchema = Yup.object({
  criminalComplaintNo: Yup.string()
    .required("Arbitration Complaint No. is required"),
  ...
  caseFilledDate: Yup.date().required("Arbitration date is required"),
...
const CreateArbitrationCriminalModal = ({ 
  isOpen, 
  onClose, 
  legal, 
  isDark, 
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const response = await legalService.createArbitrationCriminalCase(legal.id, caseData);

      // Handle HTML response (legal document)
      if (typeof response === 'string' && response.includes('<html')) {
        const blob = new Blob([response], { type: 'application/msword' });
        downloadDocument(blob, "Arbitration Criminal Case");
        toast.success("Arbitration document generated successfully!");
        if (onSuccess) onSuccess();
        setTimeout(onClose, 1000);
      } else if (response?.success) {
        toast.success(response.message || "Arbitration criminal case created successfully!");
        if (onSuccess) onSuccess();
        onClose();
      } else {
        toast.error(response?.message || "Failed to create arbitration criminal case");
      }
    } catch (error) {
      toast.error(error.message || "Failed to create arbitration criminal case");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className={`px-6 py-4 border-b ${isDark ? "border-gray-700" : "border-gray-200"}`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <Scale className={`w-6 h-6 ${isDark ? "text-pink-400" : "text-pink-600"}`} />
              <h3 className={`text-lg font-semibold ${isDark ? "text-gray-100" : "text-gray-900"}`}>
                Create Arbitration Criminal Case - {legal?.customerName}
              </h3>
            </div>
            <button
              onClick={onClose}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className={`block text-sm font-medium mb-2 ${isDark ? "text-gray-300" : "text-gray-700"}`}>
                      Arbitration Complaint No. <span className="text-red-500">*</span>
                    </label>
                    <Field
                      type="text"
                      name="criminalComplaintNo"
                      placeholder="ACC/2026/001234"
