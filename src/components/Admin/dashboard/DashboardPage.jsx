...
const DashboardPage = () => {
  ...
  const { totals, groupedStates, chartData } = useMemo(() => {
    ...
    // Disbursements: Get count from daily data (for both views), amount from current tab
    ...
    // Collections: Count and amount from current tab
    ...
    return {
      totals: {
        ...
        disbursementCount: activeTab === "monthly" ? dailyDisbursementCount : dailyDisbursementCount,
        disbursementAmount,
        collectionCount,
        collectionAmount
      },
...
<div className={`rounded-2xl p-6 ${isDark ? "bg-gray-800 border border-gray-700" : "bg-white border border-gray-200"} shadow-xl`}>
  ...
  <div className="space-y-3">
    {groupedStates.slice(0, 6).map((state, index) => {
      ...
      const getRankGradient = (rank) => {
        ...
        // Soft pastel gradients for other ranks
        const gradients = [
    ...
    {/* View More Button with Gradient */}
    {groupedStates.length > 6 && (
      <button className={`w-full py-3 rounded-xl flex items-center justify-center gap-2 transition-all duration-300 text-sm
        ${isDark 
        ...
        <span className="font-medium">View {groupedStates.length - 6} more states</span>
