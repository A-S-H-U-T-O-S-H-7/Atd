...
const SecondBankStatementDocument = ({ 
  fileName, 
  hasDoc, 
  onFileView, 
  fileLoading, 
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const documentCategory = 'second_bank_statement';
  ...
  if (hasDoc && fileName) {
    ...
    return (
      <button
        ...
        title={isLoading || isCurrentlyLoading ? "Loading..." : "View Second Bank Statement"}
      >
        {isLoading || isCurrentlyLoading ? (
  ...
  return (
    <div className="p-2 rounded-lg bg-red-100 text-red-600 flex items-center justify-center cursor-not-allowed" title="Second Bank Statement Missing">
      <FileText size={18} className="flex-shrink-0" />
...
export default SecondBankStatementDocument;