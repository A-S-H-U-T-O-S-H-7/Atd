...
const PhotoDocument = ({ fileName, hasDoc, onFileView }) => {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      alert(`Failed to load photo: ${fileName}. Please check if file exists in photo folder.`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (hasDoc && fileName) {
    return (
      <button
        ...
        title={isLoading ? "Loading..." : "View Photo"}
      >
        {isLoading ? (
  ...
  return (
    <div 
      ...
      title={`Photo Missing: ${fileName || 'No file'}`}
    >
      <FileText size={18} className="flex-shrink-0" />
