...
const AppraisalReport = ({ enquiry, onBack }) => {
  ...
  const loadAppraisalData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      console.log('🔄 Loading appraisal data for application:', enquiry.application_id);
      
      const response = await appraisalCoreService.getAppraisalReport(enquiry.application_id);
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      if (data) {
        ...
        // If no data loaded, use empty initial values
