...
const OrganizationVerification = ({ formik, onSectionSave, isDark, saving }) => {
  ...
  // Handle verified status change for organization fields
  ...
  const handleFinalReportChange = (e) => {
    ...
    formik.setFieldValue('organizationFinalReport', value);
  ...
  useEffect(() => {
    ...
    const allCriticalVerified = 
      // Fields with both Verified and Status
      formik.values.organizationVerificationStatus === 'Yes' &&
      formik.values.organizationVerificationMethod === 'Positive' &&
      formik.values.hrPhoneVerificationStatus === 'Yes' &&
      formik.values.hrPhoneVerificationMethod === 'Positive' &&
      formik.values.companyPhoneVerificationStatus === 'Yes' &&
      formik.values.companyPhoneVerificationMethod === 'Positive' &&
      formik.values.hrEmailVerificationStatus === 'Yes' &&
      formik.values.hrEmailVerificationMethod === 'Positive' &&
      // Fields with only Status (Company Website)
      formik.values.companyWebsiteStatus === 'Positive' &&
      // Fields with only Verified (HR Contact)
      formik.values.hrContactVerificationStatus === 'Yes';

    // Check if ANY field is negative
    const hasAnyNegative = 
      // Fields with both Verified and Status
      formik.values.organizationVerificationStatus === 'No' ||
      formik.values.organizationVerificationMethod === 'Negative' ||
      formik.values.hrPhoneVerificationStatus === 'No' ||
      formik.values.hrPhoneVerificationMethod === 'Negative' ||
      formik.values.companyPhoneVerificationStatus === 'No' ||
      formik.values.companyPhoneVerificationMethod === 'Negative' ||
      formik.values.hrEmailVerificationStatus === 'No' ||
      formik.values.hrEmailVerificationMethod === 'Negative' ||
      // Fields with only Status (Company Website)
      formik.values.companyWebsiteStatus === 'Negative' ||
      // Fields with only Verified (HR Contact)
      formik.values.hrContactVerificationStatus === 'No';

    // Set final report
    if (allCriticalVerified) {
      if (formik.values.organizationFinalReport !== 'Positive') {
        formik.setFieldValue('organizationFinalReport', 'Positive');
      }
    } else if (hasAnyNegative) {
      if (formik.values.organizationFinalReport !== 'Negative') {
        formik.setFieldValue('organizationFinalReport', 'Negative');
      }
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="space-y-4">
            ...
            <VerificationField
              ...
              onVerifiedChange={(value) => handleVerifiedChange('organizationVerificationStatus', 'organizationVerificationMethod', value)}
              onStatusChange={(value) => formik.setFieldValue('organizationVerificationMethod', value)}
              icon={Globe}
              verifiedField="organizationVerificationStatus"
              statusField="organizationVerificationMethod"
            />
            ...
            <VerificationField
              ...
              onVerifiedChange={(value) => handleVerifiedChange('hrPhoneVerificationStatus', 'hrPhoneVerificationMethod', value)}
              onStatusChange={(value) => formik.setFieldValue('hrPhoneVerificationMethod', value)}
              icon={Phone}
              verifiedField="hrPhoneVerificationStatus"
              statusField="hrPhoneVerificationMethod"
            />
            ...
            <div className={`p-3 rounded-lg border transition-all duration-200 ${
              ...
              <select
                ...
                onChange={(e) => {
                  formik.setFieldValue('hrContactVerificationStatus', e.target.value);
                }}
                className={selectClassName}
              >
                <option value="">Verified</option>
          ...
          <div className="space-y-4">
            ...
            <VerificationField
              ...
              onVerifiedChange={(value) => handleVerifiedChange('companyPhoneVerificationStatus', 'companyPhoneVerificationMethod', value)}
              onStatusChange={(value) => formik.setFieldValue('companyPhoneVerificationMethod', value)}
              icon={Phone}
              verifiedField="companyPhoneVerificationStatus"
              statusField="companyPhoneVerificationMethod"
            />
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                <div>
                  <select
                    ...
                    onChange={(e) => handleVerifiedChange('hrEmailVerificationStatus', 'hrEmailVerificationMethod', e.target.value)}
                    className={selectClassName}
                  >
                    <option value="">Verified</option>
                    <option value="Yes">Yes</option>
