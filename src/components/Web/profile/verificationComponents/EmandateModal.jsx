...
const EMandateModal = ({ isOpen, onClose, onSuccess, user, applicationId }) => {
  ...
  const handleResponse = (res) => {
    console.log('Paynimo Response:', res);
    ...
    if (
      ...
      // Call onSuccess with the status code
      ...
      toast.error('E-Mandate response incomplete');
      onSuccess('ERROR');
    } else {
  ...
  const initiateEMandateProcess = async () => {
    ...
    try {
      ...
      // Check for success or status field
      ...
      if (isSuccess && response.data.data) {
        ...
        let loan_no = response.data.data.loan_no || applicationId.toString();
        ...
        try {
          ...
          console.error('Paynimo Error:', pnError);
            ...
            <div className="mb-6">
              ...
              {!fetchingBanks && banks.length === 0 && mode === 'net' && (
                <p className="text-sm text-yellow-500 mt-1">
                  No banks available for this mode
                </p>
              )}
            </div>
          )}
