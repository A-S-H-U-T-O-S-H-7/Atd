...
// Step 4: Personal Details Validation Schema
const PersonalDetailsSchema = Yup.object().shape({
  ...
  dob: Yup.date()
    ...
    .min(new Date(Date.now() - 80 * 365 * 24 * 60 * 60 * 1000), "Age cannot exceed 80 years")
...
// Step 5: KYC Details Validation Schema
...
// Step 7: Service/Employment Details Validation Schema
const ServiceDetailsSchema = Yup.object().shape({
  ...
  netMonthlySalary: Yup.number()
    ...
    .test('less-than-gross', 'Net salary should be less than or equal to gross salary', function(value) {
...
// Step 9: Document Upload Validation Schema
const DocumentUploadSchema = Yup.object().shape({
  aadharFront: Yup.mixed()
    ...
    .test(
      ...
      (value) => value && ["image/jpeg", "image/jpg", "image/png", "application/pdf"].includes(value.type)
    )
    .test(
      "fileSize",
...
// Step 10: References Validation Schema
const ReferencesSchema = Yup.object().shape({
  references: Yup.array()
    ...
    .test('unique-phones', 'Phone numbers must be unique', function(references) {
    ...
    .test('unique-emails', 'Email addresses must be unique', function(references) {
      if (!references) return true;
